...
> **Reproducibility.** Every figure and table in this chapter is produced by an
> analysis that can be re-run against the raw data; Appendix 3 lists them in
> order. No value has been entered by hand, and an automated check compares
...
| Full 49-criterion tree | Design contribution; not empirically validated | Section 5.7 |
...
the data, and Section 5.3.5 rules it out.
...
### 5.3.5 The artefact is in the SBA's own records
...
### 5.3.6 Is twelve doing the work?
...
(The 0.8859 here and the 0.8894 in Section 5.3.2 differ because this probe
additionally excludes facilities with a term of zero. Both are produced by the
same analysis.)

### 5.3.7 The contamination reaches the dataset's own documentation
...
### 5.5a.1 Why this matters more than the AUC result
...
which is itself a limitation (Section 5.7) and the reason it must never be used
to price a facility.
...
justified in Section 4.3.2 on Arvanitis, Stampini and Vencatachellum (2015), who
report that development and credit concerns in development-bank appraisal are
"rather independent from each other".

As Section 2.6.2 sets out, their evidence is a *positive but non-significant*
...
longer available. Section 5.6b.2 sets out why the design decision nevertheless
...
## 5.6c Who the models work less well for
...
### 5.6c.1 What can and cannot be tested
...
against the four-fifths rule, in the form Feldman et al. (2015) formalise. On its own this is weak evidence, because the
groups have genuinely different default rates and a model that declines a
riskier group more often is doing its job. The second is error-rate disparity, which is Hardt, Price and Srebro's (2016) equality-of-opportunity criterion:
...
### 5.6c.3 A confound that had to be ruled out
...
hypothesis was therefore tested directly, not inferred, and Section 5.6d reports
what that test found, which is the opposite.

## 5.6d What the models do with information the applicant did not supply

Section 4.5 reported a completeness gate: the artefact refuses to return a band
...
threshold is the same portfolio rule as Section 5.6c: decline the riskiest 20%,
which on these applicants falls at a predicted default probability of 0.2293.
Section 5.6d.3 shows the conclusion does not depend on that choice.

### 5.6d.2 The models reward withholding
...
The distinction matters because neither model treats absence as Rubin (1976) would have it treated: as information about the mechanism that produced the gap. Median imputation, the standard alternative, fails differently rather than
...
### 5.6d.4 Why this matters for the artefact

The completeness gate was justified in Section 4.5 on safety grounds: a score
...
one. Section 5.6a bounds it independently: within ±25% perturbation ρ ≈ 0.98 and
...
a probability of default. Section 5.5a shows that even trained probabilistic
