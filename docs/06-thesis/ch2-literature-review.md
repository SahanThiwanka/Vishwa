...
> **Verification note.** Every source cited here was located through a literature
> search and its bibliographic details checked against an authoritative record.
> Bibliographic accuracy is not the same as having read the work, so the sources
> carrying substantive claims were obtained and read in full, and
> the accompanying bibliography records the status of each entry
> individually.
>
...
> (Section 2.6.2); Lessmann et al. used seven datasets, not eight, and report a tendency, not a uniform result (Section 2.2); and Cortés, Duchin and Sosyura's
> evidence comes from US residential mortgages rather than SME lending (Section 2.4).
> Each is corrected here and the correction is stated, not absorbed silently.
>
> The reading is not complete. Where this chapter says a claim is "not found in
> the searched literature", that is a statement about a non-exhaustive search and
...
It closes (Section 2.8) by stating what this study adds, which, after examining
...
produce the Z-score. Ohlson (1980) replaced discriminant analysis with
...
while a class label cannot (Section 5.5).
...
et al. (2015), who benchmarked forty-one classifiers across seven real-world
...
and Demirgüç-Kunt (2006) is the standard reference for access to finance as a
...
Stein (2002) explains why that is difficult. He distinguishes *hard*
...
experience instead of an explicit model. SharafEldin, Idrees and Ouf (2025)
...
Rezaei (2015) introduced the Best-Worst Method (BWM), which derives weights from two
...
responses. Rezaei (2016) gave a linear formulation with a unique solution, which
is the form implemented here (Section 3.5).

For this study's criteria tree the difference is decisive: 168 comparisons under
pairwise AHP against 86 under BWM (Section 3.5.1). That is the difference
...
the expert-structured scorecard fails on them (Section 5.6).
...
the two objectives apart and never issues a combined figure, which Section 5.6b
...
The distinction matters for how the non-aggregation design decision (Section 4.10) is justified.
...
collapsing them into one number. Section 5.6b tests the association directly on
...
The technical response has been post-hoc explanation: Local Interpretable Model-agnostic Explanations, or LIME (Ribeiro, Singh and
Guestrin, 2016) fits an interpretable local surrogate around a prediction, and
SHapley Additive exPlanations, or SHAP (Lundberg and Lee, 2017), assigns each
...
they sum precisely to the score (Section 4.7). That is a stronger guarantee than
...
Kapoor and Narayanan (2023) established the scale of the problem. Surveying
...
The full bibliography, with the verification status of each entry, accompanies
this thesis.
...
to finance as a growth constraint', *Journal of Banking & Finance*, 30(11), pp.
2931–2943.
...
fuzzy environment', *Fuzzy Sets and Systems*, 114(1), pp. 1–9.
...
sentiment in credit origination', *Journal of Financial Economics*, 121(2), pp.
392–413.
...
Predictions', *Advances in Neural Information Processing Systems 30*, pp.
4765–4774.
...
53, pp. 49–57. doi:10.1016/j.omega.2014.11.009
...
Roy, P.K. and Shaw, K. (2021) 'A multicriteria credit scoring model for SMEs
...
the Analytic Hierarchy Process"', *Management Science*, 36(3), pp. 259–268.

SharafEldin, M.A., Idrees, A.M. and Ouf, S. (2025) 'A Proposed Framework for
Loan Default Prediction Using Machine Learning Techniques', *International
