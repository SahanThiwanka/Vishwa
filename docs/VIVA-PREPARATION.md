...
| **Narrow elicitation sample** | "Eleven practitioners completed it, which is above the five-to-eight typical of published BWM studies, so the objection is not the count. It is the composition: ten of the eleven are from state commercial banking and the eleventh did not state an institution. That is the setting the source instrument comes from, so the weights describe that setting and I say so. What I would not claim is that they describe Sri Lankan SME lending generally, and §6.3 lists that as a principal limitation. Three of eighty-eight level-responses were excluded for inconsistency above CR 0.25, and I report which ones rather than absorbing them." |
