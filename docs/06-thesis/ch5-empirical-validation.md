...
[Table: Term roundness in the SBA FOIA 7(a) extract against the SBA National dataset]
...
(The 0.8859 here and the 0.8894 in Section 5.3.2 differ because this probe additionally
...
[Table: Discrimination under random and temporal validation, with bootstrap confidence intervals]
...
which is itself a limitation (Section 5.7) and the reason it must never be used to price
a facility.
...
justified in Section 4.3.2 on Arvanitis, Stampini and Vencatachellum (2015), who report
...
longer available. Section 5.6b.2 sets out why the design decision nevertheless stands,
...
hypothesis was therefore tested directly rather than inferred, and Section 5.6d reports
what that test found — which is the opposite.

## 5.6d What the models do with information the applicant did not supply

Section 4.5 reported a completeness gate: the artefact refuses to return a band when too
...
threshold is the same portfolio rule as Section 5.6c — decline the riskiest 20%, which
...
The completeness gate was justified in Section 4.5 on safety grounds: a score computed
...
data does not contain. What Sections 5.3 to 5.6 validates is band mapping and weighted
...
the elicited model. Elicitation completed after the analyses were run (Section 6.1), and
Section 6.1.5 measures the consequence directly: moving from the placeholder vector to
the elicited one preserves the ranking (ρ = 0.91 credit, 0.97 development) and
moves no case by two risk bands, though 13–16% move by one. Section 5.6a bounds it
...
employment only; five of the nine items in clause 5 have no counterpart. Section 5.6b
